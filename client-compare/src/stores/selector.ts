...
export enum SelectorType {
  ...
  Unsure = 'Unsure',
  /** The type of selectors that search for consensus entries. */
  Consensus = 'Consensus',
  /** The type of selectors that search for dissensus entries. */
  Dissensus = 'Dissensus',
...
const buildConsensusSelector = (
): Selector<SelectorType.Consensus> => ({
  type: SelectorType.Consensus,
  query: null,
  uuid: uuidv4(),
})

const buildDissensusSelector = (
): Selector<SelectorType.Dissensus> => ({
  type: SelectorType.Dissensus,
  query: null,
  uuid: uuidv4(),
})
...
const applySelector = (
  ...
  if (selector.type === SelectorType.Category) {
    ...
    // The UUIDs of data entries assigned the category by at least one annotator.
    const matchedUuids = new Set(profiles.value.flatMap((d) => d.annotations)
  ...
  if (selector.type === SelectorType.Unsure) {
    const { unsureUuids } = storeToRefs(useProfileStore())
    return data.filter((d) => unsureUuids.value.includes(d.uuid))
  }
  if (selector.type === SelectorType.Consensus) {
    const { consensusUuids } = storeToRefs(useProfileStore())
    return data.filter((d) => consensusUuids.value.includes(d.uuid))
  }
  if (selector.type === SelectorType.Dissensus) {
    const { dissensusUuids } = storeToRefs(useProfileStore())
    return data.filter((d) => dissensusUuids.value.includes(d.uuid))
  }
  return []
}
...
export const useStore = defineStore('selectors', {
  ...
  actions: {
    ...
    /** Add/Remove a selector of consensus data entries. */
    toggleConsensusSelector(): void {
      this.toggleSelector(buildConsensusSelector())
    },
    /** Add/Remove a selector of dissensus data entries. */
    toggleDissensusSelector(): void {
      this.toggleSelector(buildDissensusSelector())
    },
